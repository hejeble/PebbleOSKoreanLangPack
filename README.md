# PebbleOS Korean Language Pack (한국어) — Community

Korean font + translation pack (`.pbl`) for watches running the new
open-source **PebbleOS** (Core Devices **Pebble Time 2**, Pebble 2 Duo, and
older Pebbles updated to a firmware that uses the 21-slot `lang` layout).

Once installed, Korean text in notifications, calendar events, music
metadata, etc. renders in **Noto Sans KR** at all five system font sizes
(Gothic 14/18/24/28/36, regular + bold) instead of ▯▯▯ tofu boxes. A
starter set of ~90 UI strings (Settings → 설정, days/months, alarms, …) is
also translated; untranslated strings fall back to English.
...
- **Mobile app:** on builds that register the `.pbl` file type, opening the
  ...
  version but the developer-connection route above always works).
...
```
 1  STRINGS                     gettext .mo (needs Language:/Name:/Project-Id-Version: headers
                                and the msgfmt hash table)
...
`translations/ko_KR.po` holds the UI strings; msgids must match
`resources/normal/base/lang/tintin.pot` in the PebbleOS tree (~570
strings total, ~90 translated so far). PRs welcome and since PebbleOS
...
- Built against the `lang` layout in coredevices/PebbleOS as of mid-2026;
  ...
  glyph tables (hash lookup, RLE4, metrics).
